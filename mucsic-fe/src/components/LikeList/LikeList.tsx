...
const LikeListComponent = ({title, list=null}:any) => {

  const user = useAppSelector((state:any) => state.auth.user);
  const [data, setData] = useState<any>(user && user.liked || [])
  ...
  useEffect(()=>{
    ...
    if(user && !list){
      setData(user.liked.slice((limit* (currentPage - 1)),   (limit* (currentPage))))
    }
    else if(list){
      setData(list.slice((limit* (currentPage - 1)),   (limit* (currentPage))))
    }
  },[currentPage])
    ...
    <MainWrap>
      <div style={{ width: '100%', padding: '30px' }}>
        ...
        <div style={{ padding: '0 32px' }}>
          ...
          <div className="rows">
            {(user || list) && data.map((song: any, index: number) => {
              return (
                <div
                  className="row-item"
                  key={song.id}
                  ...
                  localStorage.setItem("recommendSong", JSON.stringify(user.liked));
                  ...
                  dispatch(setRecommentMusic(user.liked))
                ...
                >
                  ...
                  >
                    ...
                    >
                      <img src={`${process.env.REACT_APP_CLOUD_FRONT_URL}/${process.env.REACT_APP_IMAGE_FOLDER}/${song.image}`} style={{ width: '40px' }} />
                    </div>
                    <div className="title">
                  ...
                  >
                    ...
                    style={{cursor: "pointer", background:"transparent",
                     color: user &&  user.liked.filter((item:any) => item.id === song.id).length > 0 ? "#1ed760" : "#fff", border: "0"}}
                     onClick={(e)=>{
                      e.stopPropagation();
                      if(user && getAccessToken()){
                        if(user.liked.filter((item:any) => item.id === song.id).length === 0){
                          LikeMusic(song.id)
                        dispatch(setUser({...user, liked: user.liked.filter((item:any) => item.id !== song.id)}))
      ...
      <ReactPaginate
        ...
        pageCount={(user || list) && (list && list.length > 0 ? list.length / limit : user.liked.length / limit)}
        previousLabel="< "
